...
function collectAllTextNodes() {
    ...
    function traverse(node) {
        if (node.type === "TEXT") {
            ...
            const originalContent = textNode.getPluginData("originalText") || textNode.characters;
            const textInfo = {
                id: node.id,
...
async function generateUxWritingContent(textNodes) {
    ...
    try {
        ...
        const improvedTexts = await improveUxWritingBatch(null, // API 키 없이 모킹 사용
        texts, { tone: "friendly" }, // 기본 옵션
        (current, total) => {
...
async function toggleTextContent(nodeId, useUxWriting) {
    console.log(`🔄 텍스트 토글 시작: nodeId=${nodeId}, useUxWriting=${useUxWriting}`);
    ...
    for (const textInfo of textNodes) {
        try {
            ...
            if (!textInfo.node.getPluginData("originalText")) {
                textInfo.node.setPluginData("originalText", textInfo.originalContent || currentText);
                console.log(`💾 원본 텍스트 저장: "${textInfo.originalContent || currentText}"`);
            }
            // 번역 수행 (현재 텍스트 기준)
...
figma.ui.onmessage = async (msg) => {
    ...
    try {
        if (msg.type === "get-texts") {
            ...
            figma.notify(`${modeText}로 변경되었습니다! ${useUxWriting ? "✨" : "📝"}`);
        }
