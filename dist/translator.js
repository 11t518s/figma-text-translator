 ...
 * @returns 번역된 텍스트
 */
async function translateWithOpenAI(apiKey, request) {
    var _a, _b, _c;
        ...
        const response = await fetch("https://api.openai.com/v1/chat/completions", {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: "system",
                        content: `You are a professional translator. Translate the given text to ${getLanguageName(targetLanguage)}. Only return the translated text without any additional explanation or formatting. Maintain the original tone and style.`,
                    },
                    {
                        role: "user",
                        content: text,
 ...
 * @returns 번역된 텍스트 배열
 */
async function translateBatch(apiKey, texts, targetLanguage, onProgress) {
    const results = [];
