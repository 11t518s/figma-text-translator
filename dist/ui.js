...
function findUIElements() {
    const elements = {
        textListElement: document.getElementById("textList"),
        uxWritingListElement: document.getElementById("uxWritingList"),
        languageSelectElement: document.getElementById("languageSelect"),
        refreshBtnElement: document.getElementById("refreshBtn"),
        translateBtnElement: document.getElementById("translateBtn"),
        loadingElement: document.getElementById("loading"),
        statusElement: document.getElementById("status"),
    };
    console.log("🔍 DOM 요소 찾기 결과:");
    Object.entries(elements).forEach(([key, element]) => {
        console.log(`  ${key}:`, !!element, element === null || element === void 0 ? void 0 : element.tagName);
    });
    return elements;
}
// DOM 요소들
...
function updateOriginalTextList(texts) {
    ...
    const textItems = texts
        ...
        return `
      <div class="text-item ${activeClass}" data-id="${text.id}" data-index="${index}" title="${text.content.replace(/"/g, "&quot;")}">
...
function updateUxWritingList(uxTexts) {
    ...
    const uxItems = uxTexts
        .map((text, index) => {
        const truncatedContent = text.uxContent.length > 50
            ? text.uxContent.substring(0, 50) + "..."
            : text.uxContent;
        // 현재 텍스트가 UX 모드인지 확인
        const currentText = currentTexts.find((t) => t.id === text.id);
        const isActive = (currentText === null || currentText === void 0 ? void 0 : currentText.isUxMode) === true;
        const activeClass = isActive ? "active" : "";
        return `
      <div class="ux-item ${activeClass}" data-id="${text.id}" data-index="${index}" title="${text.uxContent.replace(/"/g, "&quot;")}">
        ${truncatedContent || "[빈 텍스트]"}
        ${isActive ? '<div class="text-item-badge">현재</div>' : ""}
      </div>
    `;
...
window.onmessage = (event) => {
    ...
    const { type, texts, uxTexts, languages, language } = event.data.pluginMessage;
